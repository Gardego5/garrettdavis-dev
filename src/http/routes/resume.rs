...
use crate::http::{
    components::{layout::margins, template::template},
    context::ValidBody,
};
...
impl Render for ItemDesc {
    fn render(&self) -> Markup {
        match self {
            ItemDesc::P(desc) => html! { p class="text-sm" { (desc) } },
            ItemDesc::Ul(list_items) => html! {
...
async fn resume() -> Markup {
    ...
    let body = html! {
        ...
        (margins(html! { div class="grid grid-cols-[1fr_6fr] [&>*:nth-child(odd)]:text-right gap-4 print:text-black max-w-4xl" {
            ...
            @for section in SECTIONS.iter() { (section) }
        }}))
    };
